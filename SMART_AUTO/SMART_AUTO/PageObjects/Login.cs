...
namespace SMART_AUTO
{
    public class Login
    {
        ...
        ///</summary>
        public Login signOutOfApplication()
        {
            Assert.IsTrue(driver._waitForElement("xpath", "//button[text()='Sign Out']"), "Sign Out Button not present");
            driver._click("xpath", "//button[text()='Sign Out']");

            return new Login(driver, test);
        }

        /// <summary>
        /// Click Button on New Login Page
        /// </summary>
        /// <param name="buttonName">Button Name to Click</param>
        /// <returns></returns>
        public Login clickButtonOnNewLoginPage(string buttonName)
        {
            if (buttonName.Contains("different email"))
            {
                Assert.IsTrue(driver._waitForElement("xpath", "//a[@class='CFT-auth-btn btn btn-default btn-block btn-outline']/span"), "Try a different email button not present.");
                driver._clickByJavaScriptExecutor("//a[@class='CFT-auth-btn btn btn-default btn-block btn-outline']/span");
            }
